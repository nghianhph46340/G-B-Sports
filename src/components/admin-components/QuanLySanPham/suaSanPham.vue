<template>
    ...
    <!-- Thêm phần hiển thị chi tiết sản phẩm -->
    <div class="mt-4">
        <h3>Chi tiết sản phẩm</h3>
        <a-table :columns="columns" :data-source="variants" :pagination="false">
            <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'action'">
                    <a-space>
                        <a-button type="primary" @click="editVariant(record)">Sửa</a-button>
                        <a-button danger @click="deleteVariant(record)">Xóa</a-button>
                    </a-space>
                </template>
            </template>
        </a-table>
        <a-button type="primary" class="mt-2" @click="showAddVariantModal">
            Thêm chi tiết sản phẩm
        </a-button>
    </div>

    <!-- Modal thêm/sửa chi tiết sản phẩm -->
    <a-modal v-model:visible="isVariantModalVisible"
        :title="editingVariant ? 'Sửa chi tiết sản phẩm' : 'Thêm chi tiết sản phẩm'" @ok="handleVariantModalOk"
        @cancel="handleVariantModalCancel">
        <a-form :model="variantForm" layout="vertical">
            <a-form-item label="Màu sắc" name="id_mau_sac"
                :rules="[{ required: true, message: 'Vui lòng chọn màu sắc!' }]">
                <a-select v-model:value="variantForm.id_mau_sac" placeholder="Chọn màu sắc">
                    <a-select-option v-for="item in mauSacList" :key="item.id_mau_sac" :value="item.id_mau_sac">
                        {{ item.ten_mau_sac }}
                    </a-select-option>
                </a-select>
            </a-form-item>
            <a-form-item label="Kích thước" name="id_kich_thuoc"
                :rules="[{ required: true, message: 'Vui lòng chọn kích thước!' }]">
                <a-select v-model:value="variantForm.id_kich_thuoc" placeholder="Chọn kích thước">
                    <a-select-option v-for="item in sizeList" :key="item.id_kich_thuoc" :value="item.id_kich_thuoc">
                        {{ item.gia_tri }}
                    </a-select-option>
                </a-select>
            </a-form-item>
            <a-form-item label="Giá" name="gia" :rules="[{ required: true, message: 'Vui lòng nhập giá!' }]">
                <a-input-number v-model:value="variantForm.gia" :min="0" style="width: 100%" />
            </a-form-item>
            <a-form-item label="Số lượng" name="so_luong"
                :rules="[{ required: true, message: 'Vui lòng nhập số lượng!' }]">
                <a-input-number v-model:value="variantForm.so_luong" :min="0" style="width: 100%" />
            </a-form-item>
        </a-form>
    </a-modal>
</template>
...
// Thêm các ref cho chi tiết sản phẩm
...
onMounted(async () => {
    try {
        ...
        // Khởi tạo fileList cho mỗi biến thể
        variants.value = (store.getCTSPBySanPhams || []).map(variant => ({
            ...variant,
            fileList: variant.hinh_anh ? variant.hinh_anh.map(url => ({
                uid: url,
                name: url.split('/').pop(),
                status: 'done',
                url: url
...
// Định nghĩa columns cho bảng chi tiết sản phẩm
const columns = [
    {
        title: 'Màu sắc',
...
// Các hàm xử lý chi tiết sản phẩm
...
const editVariant = (record) => {
    editingVariant.value = record;
    variantForm.value = { ...record };
    isVariantModalVisible.value = true;
};

const deleteVariant = async (record) => {
    try {
        await store.deleteCTSP(record.id_chi_tiet_san_pham);
        message.success('Xóa chi tiết sản phẩm thành công');
        // Reload danh sách chi tiết sản phẩm
        await store.getCTSPBySanPham(store.sanPhamById.id_san_pham);
        variants.value = store.getCTSPBySanPhams;
    } catch (error) {
        message.error('Có lỗi khi xóa chi tiết sản phẩm');
        console.error(error);
    }
};

const handleVariantModalOk = async () => {
    try {
        const variantData = {
            ...variantForm.value,
            id_san_pham: store.sanPhamById.id_san_pham
        };

        if (editingVariant.value) {
            await store.updateCTSP(variantData);
            message.success('Cập nhật chi tiết sản phẩm thành công');
        } else {
            await store.createCTSP(variantData);
        ...
        message.error('Có lỗi khi lưu chi tiết sản phẩm');
        console.error(error);
    }
};
